--
-- voicebuttonsExport.lua
-- export the mod (plane) info for using with voiceButtons
-- daisy chains other calls, so it's safe to use it.
-- Based on code of Winwing
--
g_lfs=require('lfs')
package.path  = package.path..";"..g_lfs.writedir().."Scripts\\voicebuttons\\?.lua"
package.cpath = package.cpath..";"..g_lfs.writedir().."Scripts\\voicebuttons\\?.dll"
do
	if g_voiceBInit==nil then
		...
		_voiceb.net=require("voicebuttonsNetwork")
        _voiceb.net.start()
        local _send={}
		_send["cmd"]="start"
		_send["value"]=true
		...
		do
			...
			_voiceb.LuaExportStart=function()
				log.write("voiceButtons",log.INFO,"Export start!")
			end
			...
			_voiceb.getNet=function(t)
                ...
                if data~=nil and data.Name~=nil then
                    ...
                    local _msg =  _voiceb.json:encode(_send)
                    _voiceb.net.send(_msg)
                    --log.write("voiceButtons",log.INFO,_msg)
                end
			end
			...
			_voiceb.LuaExportStop=function()
                ...
                local _msg =  _voiceb.json:encode(_send)
				_voiceb.net.send(_msg)
				log.write("voiceButtons",log.INFO,"Export stop!")
			end
            ...
            log.write("voiceButtons",log.INFO,"voiceButtons export installed!")
        end
	end
end